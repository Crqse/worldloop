...
不是一个"聊天历史上叠状态"的 Agent 框架。WorldLoop 是给那些**需要轨迹本身
...
2. **对 Agent 行为做真正的反事实实验。**
   从同一个 checkpoint 把世界分叉成只在**单一动作**上有差异的几条支线
   （"如果 leader 在第 5 tick 选择 repair 而不是 patrol 会怎样？"），
   各自确定性推进后对比结果状态。这是策略 A/B、反事实数据生成、以及
   "什么真正起了作用"这类因果问题的核心原语。

3. **生成带泄漏检查的轨迹数据集用于训练。**
   把策略（脚本 / 对抗 / LLM）调度到多个种子、做反事实分叉、导出结构化
   episode，并给出**泄漏报告**——它会标出 prompt 里的绝对路径、API key、
   缓存状态与个人身份信息（PII），让你知道下游模型到底被喂了什么。

4. **把 RL 环境（PettingZoo / Gymnasium）纳入同一套权威契约。**
   让外部 MARL 环境走同一条 提议→校验→结算→记录 管线。LLM 或策略可以
   在 Simple Spread / Simple Tag 里行动，但**永远不能绕过一条合法性或
   成本规则去直接写状态**。

如果你的工作就是"Agent 在一个循环里聊天并打印一叠记录"，你不需要
WorldLoop。如果你需要**重放、分叉、审计或导出** Agent 做过什么，并要证明
权威来源是环境而不是模型——这就是为你准备的。
